...
def confirm(token):
    try :
        ...
        cursor = mydb.cursor(buffered=True)
        cursor.execute('insert into user(Name,Mail,pwd,phno,place) values(%s,%s,%s,%s,%s)',[userinfo['name'],userinfo['mail'],userinfo['pwd'],userinfo['ph'],userinfo['place']])
        mydb.commit()
        cursor.close()
...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor = m  ydb.cursor(buffered=True)
        cursor.execute('select Mail,pwd from user where Mail=%s',[Mail])
        e_mail,password = cursor.fetchone()
        cursor.execute('select Name from user where Mail=%s',[Mail])
...
def logout():
    if session.get('user'):
        ...
        return re_direct(url_for('login'))
    else:
        return re_direct(url_for('login'))    
